...
export function* jobApplication(api, action) {
  ...
  try {
    const response = yield call(
      api,
      'post',
      `job_application/${action.job_opportunity_id}`,
    );

    if (response.status === 201) {
      yield put(JobApplicationActions.jobApplicationClearErrors());
      yield put(
        JobApplicationActions.jobApplication(
          response.data.job_application,
    ...
    yield put(
      JobApplicationActions.jobApplicationSetErrors(
...
export function* getJobApplications(api) {
  ...
  if (response.data.jobApplications.length) {
    yield put(
      JobApplicationActions.getJobApplications(response.data.jobApplications),
    );
  }
}
...
export function* saveDocument(api, action) {
  ...
  if (response.data.documents.length) {
    yield put(
      JobApplicationActions.jobApplicationSaveDocument(response.data.documents),
    );
  }
}
...
export function* getDocuments(api) {
  try {
    ...
    if (response.data.documents.length) {
      yield put(
        JobApplicationActions.jobApplicationGetDocuments(
...
export function* cancelApplication(api, action) {
  try {
    const response = yield call(
      api,
      'delete',
      `job_application/${action.jobApplicationId}`,
    );

    yield put(
      JobApplicationActions.jobApplicationSetMessage(response.data.message),
    );
    yield put(
      JobApplicationActions.getJobApplications(response.data.jobApplications),
    );
